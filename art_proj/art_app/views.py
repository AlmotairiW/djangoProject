...
def payment_success(request, artwork_id):
    context = {
        'this_artwork' : Artwork.objects.get(id = artwork_id),
...
def check_out(request, artwork_id, customer_id):
    print(artwork_id)
    print(customer_id)
    this_item = Artwork.objects.get(id = artwork_id)
    this_item.quantity = this_item.quantity - 1
    this_item.save()
    this_customer= Customer.objects.get(account_id = customer_id)
    print(this_customer.account.first_name)
    this_customer.purchases.add(this_item)
    print(this_customer.purchases.last().title)
    context = {
        'this_artwork' : Artwork.objects.get(id = artwork_id),
        'this_user' : Customer.objects.get(account_id = customer_id),
    }
    # will replaced with ckeckout page
    return render(request, "payment_success.html", context)
    # else:
    # return redirect(f'/artwork_info/{artwork_id}')
